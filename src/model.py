...
class CapsuleNetwork(tf.layers.Layer):
    def __init__(self, m_dim, dim, seq_len, bilinear_type=2, num_interest=4, hard_readout=True, relu_layer=False,tmax=100,tmin=100):
        ...
        init_tao = lambda shape, dtype, partition_info=None: 86400*tf.constant([[[1,2,7,14,30,60,365,730]]])
...
class Model_ComiRec_DR(Model):
    def __init__(self, n_mid, embedding_dim, hidden_size, batch_size, num_interest, seq_len=256, hard_readout=True, relu_layer=False):
        ...
        capsule_network = CapsuleNetwork(embedding_dim, hidden_size, seq_len, bilinear_type=2, num_interest=num_interest, hard_readout=hard_readout, relu_layer=relu_layer)
        self.user_eb, self.readout = capsule_network(item_his_emb, self.item_eb, self.t_batch_ph, self.t_his_batch_ph, self.mask)
